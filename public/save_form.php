...
// error_reporting già impostato sopra per mostrare tutti gli errori

// DEBUG: Conferma che il file si sta caricando (solo nei log)
error_log('[DEBUG] save_form.php caricato alle ' . date('Y-m-d H:i:s'));
...
// Debug: Log tutti i dati POST ricevuti (solo nei log)
error_log('[DEBUG] Dati POST ricevuti: ' . json_encode($_POST));
...
try {
    ...
    error_log("QR Code generato con phpqrcode: " . $qrFile);
...
try {
    ...
    if (!empty($eventoData['background_image'])) {
         ...
         // Debug: verifica se è un file hero invece di background
         if (strpos($raw, 'hero_') !== false) {
             error_log('[PDF] WARNING: Background image sembra essere un file hero: ' . $raw);
         }
...
<body style='margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, \"SF Pro Display\", \"Helvetica Neue\", Arial, sans-serif; background: linear-gradient(135deg, #1a1a1a 0%, #2d2d2d 50%, #1a1a1a 100%); color: #ffffff; min-height: 100vh;'>
    ...
    // Debug per Google Cloud - verifica se siamo su App Engine
    if (isset($_SERVER['GAE_APPLICATION'])) {
        error_log("Google Cloud: Invio email HTML - Subject: " . $mail->Subject);
        error_log("Google Cloud: Email Body length: " . strlen($emailBody));
        error_log("Google Cloud: isHTML: " . ($mail->isHTML() ? 'true' : 'false'));
    }
